...
useEffect(() => { const handleClickOutside = (e) => { if (containerRef.current && !containerRef.current.contains(e.target)) { setSuggestions([]); setHighlightIndex(-1); setSearchSuggestions([]); setHighlightSearchIndex(-1); } }; document.addEventListener('mousedown', handleClickOutside); return () => document.removeEventListener('mousedown', handleClickOutside); }, []);
...
const handleSuggestionClick = (ip) => { if (!inputRef.current) return;
...
useEffect(() => { if (!searchTerm.trim()) { setSearchSuggestions([]); return; }
