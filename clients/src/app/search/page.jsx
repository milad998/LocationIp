...
export default function SearchIps() {
  ...
  const updateFilterSuggestions = (value) => {
    const lastWord = value.trim().toLowerCase();
    if (!lastWord) {
      setFilterSuggestions([]);
      return;
    }
  ...
  return (
    <div className="container-fluid py-4">
      <div className="bg-white shadow rounded p-3 p-md-4 mx-auto" style={{ maxWidth: '720px' }}>
        ...
        <form onSubmit={handleSearch}>
          <div className="mb-3 position-relative" ref={containerRef}>
            <label className="form-label">أدخل عناوين IP أو أسماء</label>
            <div className="position-relative">
              <input
                className="form-control"
                type="text"
                placeholder="اكتب جزءًا من الاسم أو IP"
                value={ips}
            ...
            {suggestions.length > 0 && (
              <ul className="list-group position-absolute w-100 z-3" style={{ top: '100%', left: 0 }}>
                {suggestions.map((entry, idx) => (
                  <li
                    key={idx}
                    className={`list-group-item list-group-item-action d-flex justify-content-between ${idx === highlightIndex ? 'active' : ''}`}
                    style={{ cursor: 'pointer' }}
          ...
          <div className="mb-3 position-relative" ref={filterRef}>
            <label className="form-label">فلترة النتائج</label>
            <input
              className="form-control"
              type="text"
