...
import { useEffect, useRef, useState } from 'react'; import { Search, Loader2, CheckCircle, AlertCircle, X, Clipboard } from 'lucide-react';

const CITY_MAP = { al_raqqa: 'الرقة', al_tabaqa: 'الطبقة', kobani: 'كوباني', };

export default function SearchIps() { const [ips, setIps] = useState(''); const [filterInput, setFilterInput] = useState(''); const [result, setResult] = useState(typeof window !== 'undefined' ? localStorage.getItem('lastResult') || '' : ''); const [status, setStatus] = useState(null); const [loading, setLoading] = useState(false); const [suggestions, setSuggestions] = useState([]); const [highlightIndex, setHighlightIndex] = useState(-1); const [filterSuggestions, setFilterSuggestions] = useState([]); const [filterHighlightIndex, setFilterHighlightIndex] = useState(-1); const [allData, setAllData] = useState([]);
...
const updateFilterSuggestions = (value) => { const filterValue = value.trim().toLowerCase(); if (!filterValue || !result) { setFilterSuggestions([]); return; } const lines = result.split('\n').filter(line => line.trim() !== ''); const matches = lines .map(line => line.trim()) .filter(line => line.toLowerCase().includes(filterValue)) .map(line => ({ ip: line, name: line }));
      ...
      <div className="mb-3 position-relative" ref={containerRef}>
        ...
        <div className="position-relative">
          ...
          {ips && (
            <X
              size={18}
              className="position-absolute top-50 end-0 translate-middle-y me-2 text-danger"
      ...
      <div className="mt-4">
        <div className="d-flex justify-content-between align-items-center mb-2">
          ...
          <div className="d-flex gap-2">
            <button onClick={handleCopy} className="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1">
